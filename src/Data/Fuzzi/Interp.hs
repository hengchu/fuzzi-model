...
eval (LogBase base n) = logBase_ (eval base) (eval n)
eval (Log a) = log_ (eval a)
eval (Pow base n) = pow_ (eval base) (eval n)
eval (Exp n) = exp_ (eval n)
eval (Sqrt n) = sqrt_ (eval n)
